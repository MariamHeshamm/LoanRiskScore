# -*- coding: utf-8 -*-
"""adaboost with DT.ipynb

Automatically generated by Colaboratory.

Original file is located at
    https://colab.research.google.com/drive/1PNIJ_EMUfR6zXbprM0uo9EFaJ4o3MJ1T
"""
...
# dataframe=pd.read_csv('/content/drive/MyDrive/LoanRiskClassification.csv')
...
data = pd.read_csv('LoanRiskClassification.csv')  ###change
...
mutual_info = mutual_info_classif(old_X_train, old_Y_train)
mutual_info = pd.Series(mutual_info)
mutual_info.index = old_X_train.columns
...
model = AdaBoostClassifier(DecisionTreeClassifier(max_depth=10), random_state=1,
                           learning_rate=0.3)  # ,n_estimators=150)
print(type(old_Y_train))
old_Y_train = old_Y_train.values.flatten()  # convert dataframe to 1d array
print(type(old_Y_train))
...
model.score(old_X_train, old_Y_train)
...
valued_rows = all_test_data[all_test_data['ProsperRating (Alpha)'].notna()]
valued_rows=valued_rows.reset_index(drop=True)
X_test = valued_rows.iloc[:, 0:-1]
Y_test = valued_rows['ProsperRating (Alpha)']
'''

"""test"""

start = time.time()
accuracy = model.score(X_test, Y_test)  # Return the mean accuracy on the given test data and labels.
stop = time.time()
...
print(type(Y_test))
#y_prediction.to_frame() #AttributeError: 'numpy.ndarray' object has no attribute 'to_frame'
Y_test.to_numpy()
accuracy=np.mean(y_prediction == Y_test)*100
'''

'''Pkl_Filename = "Pickle_RL_Model.pkl"  ###change
