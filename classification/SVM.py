...
train_median_value,old_X_train[numeric_cols_with_outliers]=handling_null_values_with_median(old_X_train[numeric_cols_with_outliers])
...
train_categorical_mode_value,old_X_train[categorical_col]=handling_null_values_with_mode(old_X_train[categorical_col])
...
#model
start = time.time()
svm_kernel_ovo = SVC(kernel='linear', C=0.5).fit(old_X_train, old_Y_train)
stop = time.time()
print(f"Training time: {stop - start}s")
trainingaccuracy = svm_kernel_ovo.score(old_X_train, old_Y_train)

start = time.time()
testaccuracy = svm_kernel_ovo.score(X_test, Y_test)
stop = time.time()

print('LinearSVC OneVsOne SVM Training accuracy: ' + str(trainingaccuracy))
print(f"Testing time: {stop - start}s")
print('LinearSVC OneVsOne SVM Testing accuracy: ' + str(testaccuracy))

'''Pkl_Filename = "SVM_model.pkl"

with open(Pkl_Filename, 'wb') as f:
    pickle.dump(svm_kernel_ovo, f)

with open('median_values_for_svm.pkl', 'wb') as f:
    pickle.dump(train_median_value, f)

#with open('median_values_for_svm.pkl', 'rb') as f:
   #train_median_value = pickle.load(f)

with open('mode_values_for_svm.pkl', 'wb') as f:
    pickle.dump(train_categorical_mode_value, f)'''

#with open('mode_values_for_svm.pkl', 'rb') as f:
