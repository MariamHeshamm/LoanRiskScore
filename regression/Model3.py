...
df.drop(['BorrowerState'], axis=1, inplace=True)
df.drop(['EmploymentStatusDuration'], axis=1, inplace=True)
df.drop(['IsBorrowerHomeowner'], axis=1, inplace=True)
df.drop(['RevolvingCreditBalance'], axis=1, inplace=True)
df.drop(['BankcardUtilization'], axis=1, inplace=True)
df.drop(['AvailableBankcardCredit'], axis=1, inplace=True)
df.drop(['TotalTrades'], axis=1, inplace=True)
df.drop(['DebtToIncomeRatio'], axis=1, inplace=True)
df.drop(['IncomeRange'], axis=1, inplace=True)
df.drop(['StatedMonthlyIncome'], axis=1, inplace=True)
df.drop(['Term'], axis=1, inplace=True)
df.drop(['ListingNumber'], axis=1, inplace=True)
Number_cols = ["BorrowerAPR", "CreditScoreRangeLower", "CreditScoreRangeUpper"]
df[Number_cols] = df[Number_cols].fillna(df.median())

String_cols = ["EmploymentStatus"]
df[String_cols] = df[String_cols].fillna(df.mode().iloc[0])

df['LoanRiskScore'].fillna(df['LoanRiskScore'].mean(), inplace=True)
print(df.isna().sum(axis=0))
cols = ['LoanStatus', 'EmploymentStatus' ]  # ely homa strings h3mlhom encoding
